...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    val n = input.length
    while (a < n){
      output(a) = max(output(a-1), input(a) / a)
      a = a + 1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    var maxAngle = 0F
    var a = {if (from == 0) 1 else from}
    while (a < until){
      maxAngle = max(maxAngle, input(a) / a)
      a = a + 1
    }
    maxAngle
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val (left, right) = parallel(upsweep(input, from, (from + end) / 2, threshold), upsweep(input, (from + end) / 2, end, threshold))
      Node(left, right)
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    var a = if (from == 0) 1 else from
    while (a < until){
      if (a == from) output(a) = max(startingAngle, input(a) / a)
      else output(a) = max(output(a - 1), input(a) / a)
      a += 1
    }
  }
